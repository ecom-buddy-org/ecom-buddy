    ...
    <>
      ...
      <div className={ProductStyle.wrapper}>
        <div className={ProductStyle.filterHeader}>
          ...
          <p>
            Given below the filters can be applied to find out product which
            suits best as per criteria
          </p>
          <div>
            <div>
              <h3>Sort by</h3>
              <select
                className={ProductStyle.dropDownSorting}
                // value={sort}
        ...
        <div className={ProductStyle.Main_header}>
          ...
          <div className={ProductStyle.top_head1}>
            {productData.imageURL !== "" ? (
              <div className={ProductStyle.section}>
              ...
              <Shimmer className={ProductStyle.section} height={50} />
            )}
            {productData.category !== "" ? (
              <div className={ProductStyle.section}>{productData.category}</div>
            ) : (
              <Shimmer className={ProductStyle.section} height={50} />
            )}
            {productData.price !== "" ? (
              <div className={ProductStyle.section}>
                <div className={ProductStyle.text3}>${productData.price}</div>
              </div>
            ) : (
              <Shimmer className={ProductStyle.section} height={50} />
            )}
            {productData.manufacturer !== "" ? (
              <div className={ProductStyle.section}>
                <div className={ProductStyle.text5}>
                  {productData.manufacturer}
                </div>
              </div>
            ) : (
              ...
              <div className={ProductStyle.section}>
                <div className={ProductStyle.text5}>
                  <Rating
                    ...
                    readOnly
                  />{" "}
                  ({productData.ratings_total}) reviews{" "}
                </div>
              </div>
            ) : (
              <Shimmer className={ProductStyle.section} height={50} />
